...
import { Terminal, Home, Code, Cpu, Briefcase, Mail, FileText, Menu, X } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    <>
      ...
      {/* Navigation sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen bg-terminal/50 backdrop-blur-sm border-r border-neon-blue/20 z-40 transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
            ...
            {items.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.path}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center p-2 rounded-lg transition-colors ${isActive ? 'text-neon-blue bg-neon-blue/10' : 'text-foreground/70 hover:text-neon-blue hover:bg-neon-blue/5'
                    }`}
                >
                  <motion.div
                    whileHover={{ scale: 1.1 }}
